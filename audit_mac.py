...
def collect_system_info(progress_var):
    ...
    try:
        ...
        serial_number = subprocess.check_output(cmd, shell=True).decode().strip()
    ...
    cmd = "df -h"
    partition_info = subprocess.check_output(cmd, shell=True).decode().split("\n")[1:]
    system_info["Partitions de disque"] = ", ".join([line.split()[0] + ": " + line.split()[-1] for line in partition_info if line])
    time.sleep(0.5)
    ...
    try:
        cmd = "ls /Applications"
        installed_apps = subprocess.check_output(cmd, shell=True).decode().split("\n")
    ...
    system_info["Version macOS"] = platform.mac_ver()[0]
    system_info["Nom de la machine"] = platform.node()
    system_info["Modèle Mac"] = subprocess.check_output("sysctl -n hw.model", shell=True).decode().strip()
    time.sleep(0.5)

    # Température du processeur
    progress_var.set((11/total_steps) * 100)
    system_info["Température CPU"] = "Non disponible sur macOS"
    time.sleep(0.5)

    # Services en cours d'exécution
    progress_var.set((12/total_steps) * 100)
    cmd = "launchctl list | awk '{print $3}' | tail -n +2"
    services = subprocess.check_output(cmd, shell=True).decode().split("\n")
    ...
    cmd = "ps -A -o comm="
    active_processes = subprocess.check_output(cmd, shell=True).decode().split("\n")
